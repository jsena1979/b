...
replace(concat('Fantasía','-Art-',codigo,'-',SKUCode),'/','-') as slug,
 ...
 concat(concat('Art:',agrupacion,'-',Description),'<br />',coalesce(vm_description(0,codigo),concat(talle,' - ',color))) as product_desc,Description,
 AdditionalDescription,
 agrupacion
from ljul_articulos_tango_exp t where marca ='fantasia'
order by Description,codigo;
