DROP PROCEDURE IF EXISTS completa_parent_sku;
DELIMITER $$
CREATE PROCEDURE `completa_parent_sku`( IN provee varchar(150), IN categoria varchar(500), IN Marca  varchar(500) )
 BEGIN 
 ...
 DECLARE micursor CURSOR FOR SELECT product_sku,codigo_anterior,VM_MULTIVARIANT_FIELD_LABEL1  FROM nuevos_articulos_web where nom_prove=provee OR marca=Marca;
 DECLARE CONTINUE HANDLER FOR SQLSTATE '02000' SET no_hay_registros=1;      
 set codigo_ant_aux = (select codigo_anterior from nuevos_articulos_web where nom_prove=provee or marca= Marca limit 1);
...
# call  completa_parent_sku('SIGRY WOMAN S R L','Lenceria de Mujer/Belen');
